...
export class GraphRepositoryNews {
  static instance = null;
  static getInstance() {
    if (GraphRepositoryNews.instance == null) {
      GraphRepositoryNews.instance = new GraphRepositoryNews();
    }

    return GraphRepositoryNews.instance;
  }

  private _apollo: ApolloClient<NormalizedCacheObject>;
  get apollo() {
    if (!this._apollo) {
      const publicUri = process.env.API_URI_NEWS
...
export const GraphService = new GraphRepositoryNews();
